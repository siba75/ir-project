...
وتظهر هذه القيم عند تنفيذ أي query باستخدام BM25 أو Hybrid، ويمكن تغييرها من الواجهة. يتم تطبيق القيم المختارة وقت تنفيذ الاستعلام، وتعرض الواجهة مقارنة بين التشغيل الحالي والتشغيل السابق لنفس الاستعلام لتوضيح أثر تغيير `k1` أو `b` على ترتيب النتائج والدرجات.
...
بهذا يتم تنفيذ التمثيل الهجين مرتين كما هو مطلوب: مرة بطريقة Parallel ومرة بطريقة Serial، والمستخدم يستطيع اختيار الطريقة من الواجهة.
